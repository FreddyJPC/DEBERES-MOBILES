...
const AgeCalculator = ({ planet, seconds }: { planet: Planets, seconds: number }) => {
  const calculatedAge = age(planet, seconds);

  return <div>{`Tu edad en ${planet} es aproximadamente ${calculatedAge} años.`}</div>;
};
